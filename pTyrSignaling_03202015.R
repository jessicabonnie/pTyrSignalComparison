...
blank.frame <-data.frame(Int_n_Bk_n_A = numeric(), Int_n_Bk = numeric(), Int_n_A_n_Bk= numeric(), 
                         Sample.ID = numeric(), Status=character(), Gel = numeric())
whole.frame <- blank.frame
for (status in statuslist){
  filled.frame <- blank.frame
  status.frame <- ptyr[ptyr$Status==status,]
  for (i in seq(1,8)){ 
    subtable <- status.frame[,c(paste0(titleprefixes,".G",i),"Sample.ID","Status")]
    subtable$Gel <- i
  ...
  points(log(filled.frame[,titleprefix],base=2) ~ as.factor(filled.frame$Sample.ID),col=filled.frame$Gel)
